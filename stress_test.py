...
def network_stress():
    ...
    try:
        subprocess.run("iperf3 -s & iperf3 -c 127.0.0.1 -t 30", shell=True, check=True)
...
def mysql_stress():
    ...
    try:
        result = subprocess.run(
            "sysbench --mysql-host=192.168.1.6 --mysql-user=mysqld_exporter "
            "--mysql-password=Riya@0402 --db-driver=mysql oltp_read_write "
